...
public class CommonValidation {
	...
	public static ValidationResult validateAccount(Connection connection, int accNumber) {
		String sql = "SELECT 1 FROM KNTRSKPF WHERE KRKNTN = ?";
		...
		try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
			...
			if (rs.next()) {
				return new ValidationResult("G006", "Invalid account number.");
			} else {
				return new ValidationResult("", "Successful");
			}
		} catch (SQLException e) {
